defmodule Violacorp.Libraries.AccomplishNew do
  ...
  defp gets_http_new(url, header) do
    case HTTPoison.get(url, header, [recv_timeout: 50_000]) do
  ...
  defp put_http(url, header, body) do
    case HTTPoison.put(url, body, header, [recv_timeout: 50_000]) do
      {:ok, %{status_code: 200, body: body}} ->
        response = Poison.decode!(body)
        code = response["result"]["code"]
        if code == "5020" do
  ...
  defp delete_http(url, header, body) do
    case HTTPoison.request(:delete, url, body, header, [recv_timeout: 50_000]) do
      {:ok, %{status_code: 200, body: body}} ->
        response = Poison.decode!(body)
        code = response["result"]["code"]
        if code == "5020" do
